function [outMean, outNSample, outEVect, outEVal] = SemiMergeSt(inMean_1, inNS_1, inEVect_1, inEVal_1, inMean_2, inNS_2, inEVect_2, inEVal_2,IncS, eigenThreshold)
% -----------------------------------------------------------------------
...
AppIndex_1 = size(inEVect_1,2); %由于之前的筛选已经排序，故按顺序取�??
detaD1 = detaD1(1:AppIndex_1,1:AppIndex_1);
term1(1:size(inEVect_1,2),1:size(inEVect_1,2)) = (1-beta1)*inEVal_1+beta1*inEVal_1*detaD1;

detaD2 = diag(sum(IncS,1));
AppIndex_2 = size(inEVect_2,2);
detaD2 = detaD2(1:AppIndex_2,1:AppIndex_2);
inEVal_2 = (1-beta1)*inEVal_2 + beta1*inEVal_2*detaD2;
...
incEVect = sqrt(inEVal_2)*IncS(1:AppIndex_1,1:AppIndex_2)'*sqrt(inEVal_1);
term4 = zeros(reducedDim);
term4(:,1:size(inEVect_1,2)) = [GMatrix*incEVect; TMatrix*incEVect];

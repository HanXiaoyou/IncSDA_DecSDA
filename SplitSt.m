function [outMean, outNSample, outEVect, outEVal] = SplitSt(inMean_1, inNS_1, inEVect_1, inEVal_1, inMean_2, inNS_2, inEVect_2, inEVal_2,IncS,X ,eigenThreshold)
% Copy of SemiMergeSt_eig.m and modify
% -----------------------------------------------------------------------
...
reducedDim = size(inEVect_1, 2);
fprintf('The dimension of split problem is %8.5f\n',reducedDim)
beta1 = 1e-6; beta2 = 1e-3;

term1 = zeros(reducedDim, reducedDim);
detaD1 = diag(sum(IncS,2)); % sum of row; N_dec*1
detaD2 = diag(sum(IncS,1)); % sum of column; N_dec*2

inEVect_1TX = inEVect_1'*X;
term1(1:reducedDim,1:reducedDim) = inEVal_1;
term1 = term1 - beta1*inEVect_1TX* [detaD1,-IncS;-IncS',detaD2] *inEVect_1TX';

term2 = GMatrix*inEVal_2*GMatrix';

term3 = mGMatrix*mGMatrix'*((inNS_1*inNS_2)/(outNSample));%*outNSample)); % O( max(R_1, R)^2)
CompositeMatrix = term1 - term2 - term3 + beta2*speye(reducedDim); %  CompositeMatrix->RxR
%[U,Sigma,~] = svd(CompositeMatrix,0); % O(R^3)
...
outEVal = Sigma;
outEVect = inEVect_1 * U; % O(M (R_1+R) R)

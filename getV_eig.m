function [DiscriminativeComponents,S_N] = getV_eig(totalEigVect, totalEigVal, betweenEigVect, betweenEigVal,eigenThreshold)
% -----------------------------------------------------------------------
...
totalTermFactorZ = totalEigVect * pinv(sqrt(totalEigVal)); %O(MC^2) totalTermFactorZ->MxR_t
...
%[U_N,S_N,~] = svd(compositeMatrix,0); % O(R^3)

compositeMatrix = max(compositeMatrix,compositeMatrix');%保对称
[U_N, S_N] = eig(compositeMatrix);
[~, idx] = sort( diag(S_N), 'descend');%取最大的d个特征值，按降序排列
U_N = U_N(:,idx);
S_N = S_N(idx,idx);

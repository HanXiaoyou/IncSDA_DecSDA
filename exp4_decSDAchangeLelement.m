% Copy of exp4_decSDApure.m
% Difference: Data degradation 
% Goal:Update the laplacian matrix elements when updating to improve the
% recognition rate.
%% %---------------------Parameter setting---------------------------%
...
% DataBase = 'YaleB100x100'; train_num = 44; DataBaseIndex = 'YaleB_dec';
DataBase = 'YaleB100x100'; train_num = 44; DataBaseIndex = 'YaleB_dec';
...
%     clear fea gnd
    ...
    eigenThreshold = 0.01;
    option.degradation = 1; % data degradation
    [decr_ind_label,~] = find(decr_ind==init_ind); 
    option.decr_ind = decr_ind_label; %index of decremental data 1*N_dec
...
%     Y_train(init_unlabel) = 0;
    ...
    %save results
    % eig  %pure + wronglabel + change laplacian elements
    alg = strcat('decUSDA_',num2str(train_num));
    file = strcat(DataBase,alg,'_',num2str(group),'.mat');
%     discription = 'recompute the Laplacian matrix';

...
% Goal:Is the accuracy of indSDA lower than SRDA because the Laplacian matrix is formed differently? 
%% %---------------------Parameter setting---------------------------%
...
%     clear fea gnd
    ...
    eigenThreshold = 0.01;
    [result,t_decSDA] = decSDAL(data,option,eigenThreshold);
    %-----------------------------decSDA test-----------------------------%  
...
%     Y_train(init_unlabel) = 0;
    ...
    %save results
    alg = 'decSDAL';% eig  %pure + wronglabel  %change Laplacian

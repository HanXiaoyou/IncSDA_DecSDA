...
for group = 1:1
    ...
    eigenThreshold = 0.01;
    [result,t_incSDAIU] = incSDAIncUnlabel_eig_forS(data2,option2,eigenThreshold);
    %-----------------------------incSDA test-----------------------------%
    %use method of NN
    accuracy_incSDAIU = zeros(1,nupdate+1)
    ...
    for i = 1:nupdate+1
        ...
        [accuracy_incSDAIU(1,i),idx_predict] = max(accuracy);
        predictlabel(i,:) = sub_predictlabel(idx_predict,:);
    end
    %save results
    alg = 'incSDA_forS_NN_beta2';

...
% PHIPADE - Evaluate phi functions using diagonal Pad?approximations.
%
% SYNOPSIS:
%    phi_k                     = phipade(z, k);
%    phi_k                     = phipade(z, k, d); 
...
%   integrators using diagonal Pad?approximants.
...
%   d - Degree of diagonal Pad?approximant.  OPTIONAL.
%       Default value: d = 7.
%
% RETURNS:
%    phi_k                     =      \phi_k(z)
...
PADE_DEGR = 7;          % degree of (d,d)-Pad?approximant
atol = 1.0e-12;         % FLTEQ absolute error tolerance
rtol = 5.0e-13;         % FLTEQ relative error tolerance

% atol = 1.0e-6;         % FLTEQ absolute error tolerance
% rtol = 5.0e-6;         % FLTEQ relative error tolerance
...
[s, z] = scaled_arg(z);
Id = speye(size(z));
[phi_vals{1:k}] = scaled_pade(z, Id, d, k);     % (d,d)-Pad?approx
...
% Scaling to obtain well-behaved Pad?approximations.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Internal eval functions implementing Pad?algorithm.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Pad?eval algorithm due to Higham in
...
% Re-normalised (d,d)-Pad?coefficients for the \phi_\ell functions.
%
%%%% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%
% This code is based on the explicit (d,d)-Pad?formulae derived by W.
